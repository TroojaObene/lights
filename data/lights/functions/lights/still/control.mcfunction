...
##visualize the light
execute at @e[tag=selected] run function lights:lights/still/visualize
...
#with delete
execute if score @e[limit=1,tag=selected] Light_Id = @e[limit=1,tag=nearest_light] Light_Id run tellraw @s [{"text": "\n"},{"text": "[DELETE]","color": "red","hoverEvent":{"action":"show_text","contents":[{"text":"By clicking this, you remove the light from existance. BE AWARE!","color": "red"}]},"clickEvent": {"action":"run_command","value": "/tag @e[limit=1,sort=nearest,type=marker,tag=light,tag=still] add delete"}}]
