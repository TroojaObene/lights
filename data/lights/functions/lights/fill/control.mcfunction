...
execute as @e[type=marker,tag=fill,tag=light] if score @s Light_Id = @p[tag=controller] selected_Id run tag @s add selected

#give the tag to the closest light
execute as @s at @s run tag @e[tag=light,limit=1,sort=nearest] add nearest_light

#tellraw @s "fill light selected"

##visualize the light
execute at @e[tag=selected] run function lights:lights/fill/visualize
...
#with delete
execute if score @e[limit=1,tag=selected] Light_Id = @e[limit=1,tag=nearest_light] Light_Id run tellraw @s [{"text": "\n"},{"text": "[DELETE]","color": "red","hoverEvent":{"action":"show_text","contents":[{"text":"By clicking this, you remove the light from existance. BE AWARE!","color": "red"}]},"clickEvent": {"action":"run_command","value": "/tag @e[limit=1,sort=nearest,type=marker,tag=light,tag=fill] add delete"}}]
